...
export class AiReplyGeneratorService {
  ...
  async generate(input: AiReplyGenerateInput): Promise<AiReplyGenerateResult> {
    ...
    const antiHallucinationRules =
      ...
      "2. หากข้อมูลจาก Knowledge Base เป็น 'ไม่มี' หรือไม่มีข้อมูลใดๆ ที่ระบุถึงสิ่งที่ลูกค้าถามถึงโดยตรง (เช่น ถามถึงโปรโมชั่น สมัครสมาชิก หรือปัญหาการฝากเงิน แต่ไม่มีระบุในระบบอ้างอิง) ห้ามเดา ห้ามตอบโดยใช้ความรู้ทั่วไป และห้ามนำหัวข้ออื่นที่ไม่เกี่ยวข้องกับคำถามลูกค้ามาตอบแทนเด็ดขาด\n" +
      "3. ในกรณีที่หาข้อมูลไม่พบหรือไม่มีข้อมูลใน Knowledge Base ให้ปฏิเสธอย่างสุภาพเป็นภาษาไทย (เช่น 'ขออภัยด้วยค่ะ/ครับ ปัจจุบันทางระบบยังไม่มีข้อมูลเกี่ยวกับเรื่องนี้ค่ะ/ครับ') ห้ามตอบข้อมูลสมมติใดๆ ทั้งสิ้น ห้ามลืมใส่ [CONFIDENCE: x.xx] ไว้บรรทัดแรกสุดเด็ดขาด";
